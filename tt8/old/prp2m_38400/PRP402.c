...
void	SendDataFile(void);	// Send out the data file as a packet
...
/**************************************************************************/
{
	...
	ulong	cycleusecs, horizusec; //the time in usec from nadir to hemisphere
	time_t now;
	...
	Baud_Rate = SerGetBaud(38400, 16000000);  //Read Serial Baud Rate
...
***************************************************/
{
	...
	*******************************/
	...
	else ulng = (int)(100*Tilt2.pitch) + 9000;
	PackBytes(ulng, 2, 0);
	if( Tilt2.roll == Missing ) ulng = 0;
	else ulng = (int)(100*Tilt2.roll) + 9000;
	PackBytes(ulng, 2, 0);
	if( Tilt2.azimuth == Missing ) ulng = Missing;
	else ulng = (int)(100*Tilt2.azimuth);
	PackBytes(ulng, 2, 0);
	...
	************************/
	if(RunMode == HIGH || RunMode == TRANSITION)
	{
		...
		for(ic=0; ic<7; ic++) PackBytes( globalSweep[0][ic], 2,0);

		// GLOBAL SWEEP VALUES
		for(ic=0; ic<7; ic++) PackBytes( globalSweep[1][ic], 2, 0);
...
void PackBytes(unsigned long ul, unsigned int nbytes, unsigned prt)
/************************************
Pack unsigned ints to 4 bytes into the data file
Increment df_index accordingly
991101
*************************************/
{
	int	i;

	if(prt) printf("Pack %d bytes at index %d, ulng=%lX (%ld)\n",
		nbytes, df_index, ul, ul);
	for(i=0; i<nbytes; i++)
	{
		datafile[df_index] = ul%256;
		if(prt) printf("byte %d: %02X\n",i,datafile[df_index]);
		df_index++;
		ul /= 256;
	}
	return;
