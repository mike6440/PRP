...
while 1 {
	...
	if { [expr $dt - $timemsg] > 60 && [expr $dt - $timerad] > $raddeadtime} {
		send_user "NO RAW RAD since [timestamp format -format "%y%m%d,%H%M%S" -seconds $timerad ].\n"
		set timemsg $dt
	}
	...
	if {$day1 != $day0} {
		...
		if {[file exists $rawname]} {
			...
			# ARM HOUR FILE NAME
	set hour1 [timestamp -gmt -format "%y%m%d%H" -seconds $dt]
	...
	expect { 
		...
		-re "(\\\$WIR\.*?)\r\n" {
			set timerad [timestamp]    ;# exact time the packet is received.
					# --- TRAP DUP TIMES ----
			while { $timerad <= $timeradlast } {
			...
			set rawstr $Nrecs[timestamp -gmt -format " %Y %m %d %H %M %S " -seconds $timerad]$radstr
