#!/usr/bin/perl -w
#$HCHDM,280.7,MP4.8R1.8X-2.56Y21.68Z51.26T31.5*0A
#$C173.1P0.8R-0.2X-61.02Y-7.38Z62.84T19.0*4A
#$C73.1P0.8R-0.2X-61.02Y-7.38Z62.84T19.0*4A
#Proctcm.pl receives raw strings from the TCM2.5 (old style) tilt-compass and averages
#the input variables.
my $PROGRAMNAME = 'ProcTcm.pl';
my $VERSION = '01';  
my $EDITDATE = '090928';
...
my $fnavout = "tcm\_av$avgsecs\_$str.dat";
...
$sn = FindInfo($setupfile,'TCM SERIAL NUMBER', ': ');
$header = $header."TCM SERIAL NUMBER: $sn\n";
...
$Nsamp_min = FindInfo($setupfile,'TCM MIN SAMPLES FOR AVG', ': ');
$header = $header."TCM MIN SAMPLES FOR AVG: $Nsamp_min\n";

$Tabs = 273.16;  # absolute temperature at 0degC
$header = $header."KELVIN CONVERSION: $Tabs\n";

@strings = FindLines($setupfile, 'TCM COMMENTS', 100 );
$header = $header."TCM COMMENTS:\n";
if ( $#strings > 0 ){
	for($i=1; $i<=$#strings; $i++) { 
...
#$HCHDM,280.7,MP4.8R1.8X-2.56Y21.68Z51.26T31.5*0A
@VARS = ('compass', 'pitch', 'roll', 'Xmag', 'Ymag', 'Zmag', 'Ttcm', 'Xc','Yc');
...
#$C173.1P0.8R-0.2X-61.02Y-7.38Z62.84T19.0*4A
{
	...
	# print "Before: $str\n";
		...
		#print "tcmproc190 $str\n"; # test
		@dat = split(/[CPRXYZT*]/, substr($str,2) );							# parse the data record
		...
		if ( $#dat >= $Nfields -1 ) {          # PROCESS DOS OR UNIX
			...
			# VECTOR AVERAGE THE COMPASS READING.
			($Xcomp,$Ycomp) = VecP2V(1,$dat[0], $missing);
			...
			#======================
			if ( $record{compass} < 0 || $record{compass} > 360 ) { $record{compass} = $missing; }
			if ( $record{pitch} < -40 || $record{pitch} > 40 ) { $record{pitch} = $missing; }
			if ( $record{roll} < -40 || $record{roll} > 40 ) { $record{roll} = $missing; }
			if ( $record{Xmag} < -500 || $record{Xmag} > 500 ) { $record{Xmag} = $missing; }
			if ( $record{Ymag} < -500 || $record{Ymag} > 500 ) { $record{Ymag} = $missing; }
			if ( $record{Zmag} < -500 || $record{Zmag} > 500 ) { $record{Zmag} = $missing; }
			
			## RAW RT LINE
			$str = sprintf"%s, %03d,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f,%4.1f",
			dtstr($record{dt},'csv'), $record{compass}, $record{pitch}, $record{roll},
...
#*************************************************************/
sub OpenOutFile
#   DATE                Nav   Comp       P      R      X      Y        Z      T   SigC   SigP   SigR
#2009,08,04,20,06,00,    1,   280.7,   4.8,   1.8,  -2.6,  21.7,    51.3,   0.0,   0.0,   0.0,   0.0
{
	...
	open(TCM, ">$outfile") or die("Can't open out file\n"); 
	print TCM "RAW TCM DATA
$header
======================================================
DATE -- sample time, UTC
Compass -- flux gate compass, vector avg, degM
Pitch -- Instrument pitch, + bow up, deg
Roll -- Instrument roll, + port up, deg
Xmag -- X magnetic field, mV
Ymag -- KT15 BB2 output mean voltage, mV
Zmag -- KT15 calibration view mean output voltage, mV
Ttcm -- ocean emissivity used to compute Tskin
=============================
   DATE                 N  Nsamp    Comp      P      R      X      Y      Z      T     SigC   SigP   SigR
                                       M    deg    deg     mv     mv     mv      C      deg    deg    deg\n";
	close(TCM);
}
...
sub SaveStats
{
	...
	# GET VECTOR AVERAGED COMPASS READING
	($dum,$vec_compass) = VecV2P( $samp_Xc{mn}, $samp_Yc{mn}, $missing );
	#print "vecav compass=$vec_compass, scalar avg compass = $samp_compass{mn}\n";
	
	$Nsamp++;	
	#printf"Write AVG string: time = %s\n", dtstr($dt_samp,'csv');
	
	open(D,">>$outfile") or die("<<WRITE TO AVG OUT FAILS>>\n");
	## OPERATION OUT
	$str = sprintf "%s,%5d,%5d,  %6.1f,%6.1f,%6.1f,%6.1f,%6.1f,%6.1f,%6.1f,  %6.1f,%6.1f,%6.1f",
	dtstr($dt_samp,"csv"), $Nsamp, $samp_compass{n}, $vec_compass, $samp_pitch{mn}, $samp_roll{mn}, $samp_Xmag{mn}, $samp_Ymag{mn}, $samp_Zmag{mn}, $samp_Ttcm{mn},
	$samp_compass{std}, $samp_pitch{std}, $samp_roll{std};
